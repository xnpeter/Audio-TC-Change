  ...
  const LtcWorkerDecoder = {
    ...
    chooseSoftSyncCandidate(channel, record, fps, stats, expectedHalfBitSamples, strictDrop = true) {
      ...
      for (const candidate of candidates) {
        ...
        const softCandidate = { ...candidate, frames: candidate.frames.toString(), newTimeReference: candidate.newTimeReference.toString(), confidence, lockedFrames: 1, softSync: true, softSyncSupport: cluster.length, softSyncMargin: clusterMargin, diagnostics: { peak: stats.peak, rms: stats.rms, p2p: stats.p2p, decodedBits: 80, rejectedEdges: 0, observedJitter: 0, measuredHalfBitSamples: expectedHalfBitSamples, halfBitError: 0.006, rejectRatio: 0, softSyncSupport: cluster.length, softSyncMargin: clusterMargin, windowStart: stats.windowStart || 0, windowEnd: stats.windowEnd || stats.data?.length || 0 } };
        ...
        if (!best || softCandidate.softSyncSupport > best.softSyncSupport || (softCandidate.softSyncSupport === best.softSyncSupport && softCandidate.softSyncMargin > best.softSyncMargin)) best = softCandidate;
      }
      return best;
    },
    ...
    detectOnChannelData(record, channelIndex, fps, channel, expectedHalfBitSamples, strictDrop = true, allowSoftSync = true) {
      let best = null;
    ...
    detectAuto(buffer, record, preferredValue, values) {
      ...
      for (let channelIndex = 0; channelIndex < record.channels; channelIndex++) {
        ...
        if (quick.reject) {
          ...
          if (conditionedQuick.reject) { rejectedChannels.push({ channelIndex, channelLabel: String(channelIndex + 1), rejectReason: quick.reason }); continue; }
        }
